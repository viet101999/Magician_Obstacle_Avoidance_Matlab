function Lecture22PotentialFieldNavigationAnswer(choice)
...
% and Vidyasagar
%
% Aaron T Becker, 04-13-2016 --  11/14/2019, atbecker@uh.edu
%
%  This solution uses code by Mary Burbage
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if nargin<1 || choice  == 1
    choice = 1;
    disp('A fun 4-link example with point obstacles')
    linkLen = [.5;.5;.5;.5;.5];  %lengths of each link
    q = [pi/2;0;pi/4;0;0]; %robot configuration (set)
    qGoal = [-pi/2;0;-pi/2;0;pi/4];
...
if 2 == choice
    disp('two-link robot that gets stuck in a local minimum and must do a random walk to escape')
    linkLen = [1.75;1.05];  %lengths of each link
    q = [pi/2;pi/4]; %robot configuration (set)
    qGoal = [-pi/2;-pi/2];
    rhoNot = 1/2; %: defines the distance of influence of the obstacle
    isPolygonObs = false; % if true, uses polygonal obstacles
elseif 3 == choice
    disp('This local minimum is very deep, requiring a LONG random walk to escape.  It doesnt always escape (SOMETIMES IT IS STUCK FOREVER).')
    linkLen = [2;4];  %lengths of each link
    q = [pi/2;pi/4]; %robot configuration (set)
    qGoal = [-pi/2;-pi/2];
    rhoNot = 1/2; %: defines the distance of influence of the obstacle
    isPolygonObs = false; % if true, uses polygonal obstacles
...
elseif 4 == choice
    disp('polygonal obstacles, does hit a local minimum, but usually escapes quickly')
    linkLen = [1.8;1.8];  %lengths of each link
    q = [pi/2;pi/4]; %robot configuration (set)
    qGoal = [-pi/2;-pi/2];
    rhoNot = 1/2; %: defines the distance of influence of the obstacle
    isPolygonObs = true; % if true, uses polygonal obstacles
elseif 5 == choice
    disp(' 4-link robot, polygonal obstacles, has chatter that makes it take time to settle')
    linkLen = [.5;.5;.5;.5;.5];  %lengths of each link
    q = [pi/2;0;pi/4;0;0]; %robot configuration (set)
    qGoal = [-pi/2;0;-pi/2;0;pi/4];
    rhoNot = 1; %: defines the distance of influence of the obstacle
    isPolygonObs = true; % if true, uses polygonal obstacles
end
...
% draw obstacles
if isPolygonObs
    ptObstacles = [1.1,2; -3,-1]; %#ok<UNRCH>
